...
export class Truyengihot extends Source {
    ...
    async getMangaDetails(mangaId: string): Promise<Manga> {
        ...
        if (!image.includes('http')) image = 'https://truyengihotne.net/' + image;
    ...
    async getChapters(mangaId: string): Promise<Chapter[]> {
        ...
        for (var i = 0; i <= el.length - 1; i++) {
            var e = el[i];
            let id = 'https://truyengihotne.net/' + $(e).attr("href");
    ...
    async getHomePageSections(sectionCallback: (section: HomeSection) => void): Promise<void> {
        ...
        let request = createRequestObject({
            url: 'https://truyengihotne.net/danh-sach-truyen.html?page=1',
            method: "GET",
        });
        let data = await this.requestManager.schedule(request, 1);
        let $ = this.cheerio.load(data.data);
        ...
        for (var i in allItem) {
            ...
            if (!image.includes('http')) image = 'https://truyengihotne.net/' + image;
            let id = 'https://truyengihotne.net' + $('.title a', item).attr('href');
        ...
        for (const element of $('#swiper-hot .swiper-slide a').toArray()) {
            ...
            if (!image.includes('http')) image = 'https://truyengihotne.net/' + image;
            let id = 'https://truyengihotne.net' + $(element).attr('href');
    ...
    async getViewMoreItems(homepageSectionId: string, metadata: any): Promise<PagedResults> {
        ...
        switch (homepageSectionId) {
            case "new_updated":
                url = `https://truyengihotne.net/danh-sach-truyen.html?page=${page}`;
                select = 1;
                break;
            default:
    ...
    async getSearchTags(): Promise<TagSection[]> {
        ...
        const request = createRequestObject({
            url: 'https://truyengihotne.net/danh-sach-truyen.html',
            method: "GET",
        });
        let data = await this.requestManager.schedule(request, 1);
        let $ = this.cheerio.load(data.data);
